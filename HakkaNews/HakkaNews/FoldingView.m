...
- (id)initWithFrame:(CGRect)frame request:(NSURLRequest*)request
{
    ...
    if (self) {
        ...
        [_forwardButton setBackgroundImage:[UIImage imageNamed:@"forward1.png"] forState:UIControlStateNormal];
...
-(void)closeWithVelocity:(CGFloat)velocity{
    ...
    [scaleToCloseAnimation setCompletionBlock:^(POPAnimation *anim, BOOL finished) {
        if (finished) {
            UITableView *table=(UITableView*)[self.superview viewWithTag:1] ;
